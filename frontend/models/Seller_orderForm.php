...
class Seller_orderForm extends Model
{
	...
	public function getConditions($post, $query = null)
	{
		...
		//var_dump($user);die;
		//$query=$query->left('user','o.userid = user.goods_id')->where('user.userid');
		//$query=$query->leftJoin(UserModel::tableName().' as u','o.buyer_id = u.userid');
		...
		if($post->buyer_nick_name) {
			$query->andWhere(['like', 'buyer_name', $post->buyer_nick_name]);
		}
		if($post->buyer_real_name) {
			//var_dump($post->buyer_real_name);
			$user =  UserModel::find()->select('userid')->where(['like', 'real_name', $post->buyer_real_name])
			->asArray()->one();	
			//->createCommand()->getRawSql();
			//var_dump($user);die;
			$query->andWhere(['=', 'buyer_id', $user['userid']]);
		}
